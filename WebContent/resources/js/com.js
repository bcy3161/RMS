 ...
 */
	...
	function centerwin(path, w, h, scroll){
		...
		nWin = window.open(path,'nWin',making)//새창으로 나타날 문서의 경로 지정
		nWin.focus();
	}
    ...
    function fnOpenDocu(url){
    	window.open(url,"document","width=800, height=700, toolbar=no, menubar=no, resizable=no, location=no, titlebar=no");
    }
   ...
   //문자 바꾸기, 사용법 var str = 문자열.replaceAll("a", "1");
	...
	//자바스크립트 디버깅시 로그 보기
	//페이지에 <div id='log'></div> 가 있으면 여기에 로그를 기록
	//없으면 window.status에 로그를 기록한다.
	function log(txt){
